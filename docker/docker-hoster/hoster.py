...
def get_container_data(dockerClient, container_id):
    ...
    container_labels = info["Config"]["Labels"]
    ...
    for values in info["NetworkSettings"]["Networks"].values():
        ...
        tmpdomains = [container_hostname, container_name]

        if container_domainname:
            tmpdomains.append(container_domainname)

        if "domains" in container_labels:

            for x in container_labels["domains"].split(','):
                tmpdomains.append(x)

        result.append({
                "ip": values["IPAddress"] ,
                "name": container_name,
                "domains": set(tmpdomains)
            })
